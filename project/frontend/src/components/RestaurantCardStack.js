...
function RestaurantCardStack(props) {
  ...
  const cards = props.cards.map((card, index) => {
    ...
    const offset = (relativeIndex === 0) ?
          offsetBeforeUncollapsedCard : (relativeIndex > 0) ?
          offsetBeforeUncollapsedCard + uncollapsedCardHeight +
          Math.min(relativeIndex, numberOfCollapsedCards) * offsetBetweenCollapsedCards :
          Math.max(numberOfCollapsedCards + relativeIndex, 0) * offsetBetweenCollapsedCards;
    const top = offset + 'px';
    const opacity = (relativeIndexAbs <= numberOfCollapsedCards) ? 1 : 0;
    const zIndex = props.cards.length - relativeIndexAbs;
    const transform = 'scale(' +
          Math.min(Math.max(1 - relativeIndexAbs / 20, minScaleFactor), maxScaleFactor) +
          ')';
    const transition = transitionString;
    const style = { position: 'absolute', top, opacity, zIndex, transform, transition };
  ...
  return (
    <div className='restaurant-card-stack'>
      <div className='restaurant-card-stack-wrapper'>
        {cards}
      </div>
